...
const CARD_ICONS={
  ...
  tiktok:'<svg viewBox="0 0 24 24"><path d="M16.6 5.82c-1.1-.96-1.72-2.33-1.72-3.82h-3.09v12.4a2.59 2.59 0 0 1-2.59 2.5c-1.42 0-2.6-1.16-2.6-2.6 0-1.72 1.66-3.01 3.37-2.48V8.75c-3.45-.46-6.47 2.22-6.47 5.64 0 3.33 2.76 5.7 5.69 5.7 3.14 0 5.69-2.55 5.69-5.7V9.01a7.35 7.35 0 0 0 4.3 1.38V7.3c-1.4 0-2.51-.62-3.58-1.48Z"/></svg>',
  whatsappGroup:'<svg viewBox="0 0 24 24"><path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.79.47 3.53 1.36 5.07L2 22l5.25-1.38a9.9 9.9 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.86 9.86 0 0 0 12.04 2Zm0 1.67c2.11 0 4.1.82 5.59 2.31a7.9 7.9 0 0 1 2.32 5.59c0 4.36-3.55 7.91-7.91 7.91h-.01a8.2 8.2 0 0 1-4.19-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.16 8.16 0 0 1-1.25-4.35c0-4.36 3.55-7.91 7.91-7.91Zm-4.5 3.4c-.17 0-.44.06-.67.31-.23.25-.87.85-.87 2.07 0 1.22.89 2.4 1.01 2.57.12.17 1.75 2.67 4.24 3.74.59.26 1.06.41 1.42.53.6.19 1.14.16 1.57.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.08.15-1.18-.06-.1-.23-.16-.48-.28-.25-.12-1.47-.72-1.7-.8-.23-.08-.4-.12-.56.12-.17.25-.64.8-.79.97-.14.17-.29.18-.54.06-.25-.13-1.05-.39-2-1.23-.74-.66-1.24-1.47-1.39-1.72-.14-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.12-.14.16-.25.24-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.35-.77-1.85-.2-.48-.41-.42-.56-.42Z"/></svg>'
};
...
function render(){
  ...
  grid.innerHTML=result.map(s=>{
    ...
    return `<article class="supplier-card"><div class="supplier-logo" aria-label="Identidade de ${s.name}"><span>${initials(s.name)}</span><small>fornecedor parceiro</small><img class="supplier-seal" src="assets/brand/seal-parceiro.png" alt="Selo Parceiro do Universo" /></div><h3>${s.name}</h3><button type="button" class="v2-category" data-category="${s.category}">${s.category}</button>${s.description?`<p class="v2-benefit-text">${s.description}</p>`:''}<span class="v2-benefit-pill">${s.benefit}</span><div class="v2-actions"><a class="v2-whatsapp" href="${waLink(phone)}" target="_blank" rel="noreferrer" aria-label="Abrir WhatsApp de ${s.name}">WhatsApp</a><div class="v2-icon-row">${cardIconLink('instagram',igLink(s.handle),`Instagram de ${s.name}`)}${cardIconLink('tiktok',s.tiktok,`TikTok de ${s.name}`)}${cardIconLink('site',s.site,`Site de ${s.name}`)}${cardIconLink('address',s.address,`Endereço de ${s.name}`)}${cardIconLink('gmb',s.gmb,`Google Meu Negócio de ${s.name}`)}${cardIconLink('whatsappGroup',s.whatsappGroup,`Grupo do WhatsApp de ${s.name}`)}</div></div></article>`;
  }).join('');
